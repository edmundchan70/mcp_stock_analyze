  ...
  return (
    <div className="panel overflow-auto">
      <table className="w-full text-left text-xs">
        <thead>
          <tr className="border-b border-ink-800 bg-ink-900 text-slate-500">
            {columns.map((c) => (
        ...
        <tbody>
          {table.rows.map((r) => (
            <tr key={String(r.symbol)} className="border-b border-ink-800/60 text-slate-400">
              {columns.map((c) => (
...
function formatCell(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (Math.abs(value) >= 1e3) return fmtCompact(value);
