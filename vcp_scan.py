...
@dataclass
class VCPSetup:
    """Represents a complete VCP setup."""
...
def find_swing_points(
    ...
    '''
   For each day:
    Look at a window: 5 days before and 5 days after (11 days total).
    Ask: "Is today the highest (or lowest) in this window?"
    If yes → it's a new high/low → mark as a pivot.
    '''
...
def scan_vcp_from_dataframe(df: pd.DataFrame, lookback_periods: int = 3) -> Optional[VCPSetup]:
    ...
    if isinstance(df.index, pd.DatetimeIndex):
        ...
        # The reset_index() will create a column with the index name or 'index'
        df = df.reset_index()
    ...
    volatility_contracting = (
        ...
        contractions[2].depth < contractions[1].depth < contractions[0].depth
    )
    volume_contracting = (
        len(contractions) >= 3 and
        contractions[2].avg_volume < contractions[1].avg_volume < contractions[0].avg_volume
    )
...
def analyze_vcp_for_agent(
    ...
    try:
        ...
        sorted_contractions = sorted(setup.contractions, key=lambda x: x.number)
